//  DGCBEEffectManager.m
//  Core


#import "DGCBEEffectManager.h"
...
@interface DGCBEEffectManager () <RenderMsgDelegate> {
...
@implementation DGCBEEffectManager
...
- (int)initTask {
    _effectOn = true;
    _glContext = [EAGLContext currentContext];  //  {zh} 运行在主线程，使用的是self.glView.context  {en} Running on the main thread, using self.glView.context
    if (_glContext == nil) {
...
- (BOOL)setRenderCacheTexture:(NSString *)key buffer:(DGCBEBuffer *)buffer {
...
- (void)enableAlgorithmPictureMode:(BOOL)picture_mode
{
    bef_effect_result_t ret = bef_effect_ai_enable_algorithm_picture_mode(_handle, picture_mode);
    CHECK_RET_AND_RETURN_RESULT(bef_effect_ai_enable_algorithm_picture_mode, ret, ;)
}
