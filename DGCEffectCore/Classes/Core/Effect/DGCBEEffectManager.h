...
@interface DGCBEEffectManager : NSObject
...
//   {zh} / 一般情况下可以直接使用项目中的 DGCBEEffectResourceHelper 。     {en} In general, you can directly use the DGCBEEffectResourceHelper in the project. 
...
//   {zh} / @details 传入一个固定名字的纹理给到 SDK，传入 DGCBEBuffer，SDK 会将其解析成纹理     {en} /@details pass a texture with a fixed name to the SDK, pass DGCBEBuffer, and the SDK will parse it into a texture 
//   {zh} / @param key 纹理名称     {en} /@param key texture name 
//   {zh} / @param buffer DGCBEBuffer, 仅支持 RGBA 格式     {en} /@param buffer DGCBEBuffer, only supports RGBA format 
