...
@interface DGCBEOpenGLProgram : NSObject {
    GLuint          _program;
    GLuint          _position;
    GLuint          _color;
}
...
@implementation DGCBEOpenGLProgram
...
@interface DGCBEResizeTextureProgram : DGCBEOpenGLProgram {
...
@implementation DGCBEResizeTextureProgram
...
@interface DGCBEOpenGLRenderHelper () {
    DGCBEResizeTextureProgram      *_dgc_resizeTextureProgram;
...
@implementation DGCBEOpenGLRenderHelper
...
- (void)dealloc
{
    [_dgc_resizeTextureProgram destroy];
}

- (void) loadResizeShader{
    _dgc_resizeTextureProgram = [[DGCBEResizeTextureProgram alloc] initWithVertex:CAMREA_RESIZE_VERTEX fragment:CAMREA_RESIZE_FRAGMENT];
    [self checkGLError];
}
...
- (void)textureToImage:(GLuint)texture withBuffer:(unsigned char *)buffer Width:(int)rWidth height:(int)rHeight format:(GLenum)format rotation:(int)rotation {
    glViewport(0, 0, dgc_viewWidth, dgc_viewHeight);
    [_dgc_resizeTextureProgram textureToImage:texture withBuffer:buffer withWidth:rWidth height:rHeight withFormat:format withRotation:rotation];
    [self checkGLError];
}
