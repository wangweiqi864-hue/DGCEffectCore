...
@interface DGCBEPixelBufferInfo : NSObject
...
@interface DGCBEBuffer : NSObject
...
@interface DGCBEImageUtils : NSObject
...
- (DGCBEPixelBufferGLTexture *)getOutputPixelBufferGLTextureWithWidth:(int)width height:(int)height format:(BEFormatType)format withPipeline:(BOOL)usepipeline;
...
//   {zh} / @brief CVPixelBuffer 转 DGCBEBuffer     {en} /@Briefing CVPixelBuffer to DGCBEBuffer 
/// @param pixelBuffer CVPixelBuffer
//   {zh} / @param outputFormat 输出 DGCBEBuffer 格式     {en} /@param outputFormat output DGCBEBuffer format 
- (DGCBEBuffer *)transforCVPixelBufferToBuffer:(CVPixelBufferRef)pixelBuffer outputFormat:(BEFormatType)outputFormat;

//   {zh} / @brief CVPixelBuffer 转 纹理     {en} /@Briefing CVPixelBuffer, texture 
/// @param pixelBuffer CVPixelBuffer
- (DGCBEPixelBufferGLTexture *)transforCVPixelBufferToTexture:(CVPixelBufferRef)pixelBuffer;
...
#pragma mark - DGCBEBuffer to others

//   {zh} / @brief DGCBEBuffer 转 CVPixelBuffer     {en} @Briefing DGCBEBuffer to CVPixelBuffer 
/// @param buffer DGCBEBuffer
//   {zh} / @param outputFormat 输出格式     {en} /@param outputFormat output format 
- (CVPixelBufferRef)transforBufferToCVPixelBuffer:(DGCBEBuffer *)buffer outputFormat:(BEFormatType)outputFormat;

//   {zh} / @brief DGCBEBuffer 转 CVPixelBuffer     {en} @Briefing DGCBEBuffer to CVPixelBuffer 
//   {zh} / @details 将 DGCBEBuffer 的内容复制到已存在的目标 CVPixleBuffer 中，可以同时进行格式转换     {en} /@details Copy the contents of the DGCBEBuffer to the existing target CVPixleBuffer, which can be formatted at the same time 
/// @param buffer DGCBEBuffer
//   {zh} / @param pixelBuffer 目标 CVPixelBuffer     {en} /@param pixelBuffer Target CVPixelBuffer 
- (BOOL)transforBufferToCVPixelBuffer:(DGCBEBuffer *)buffer pixelBuffer:(CVPixelBufferRef)pixelBuffer;

//   {zh} / @brief DGCBEBuffer 转 DGCBEBuffer     {en} /@Briefing DGCBEBuffer to DGCBEBuffer 
//   {zh} / @param inputBuffer 输入BEBuffer     {en} /@param inputBuffer Enter DGCBEBuffer 
//   {zh} / @param outputFormat 输出格式     {en} /@param outputFormat output format 
- (DGCBEBuffer *)transforBufferToBuffer:(DGCBEBuffer *)inputBuffer outputFormat:(BEFormatType)outputFormat;

//   {zh} / @brief DGCBEBuffer 转 DGCBEBuffer     {en} /@Briefing DGCBEBuffer to DGCBEBuffer 
//   {zh} / @details 将 DGCBEBuffer 的内容复制到已存在的目标 DGCBEBuffer 中，可以同时进行格式转换     {en} /@details Copy the contents of the DGCBEBuffer to the existing target DGCBEBuffer, and format conversion can be performed at the same time 
//   {zh} / @param inputBuffer 输入 DGCBEBuffer     {en} /@param inputBuffer Enter DGCBEBuffer 
//   {zh} / @param outputBuffer 输出 DGCBEBuffer     {en} /@param outputBuffer output DGCBEBuffer 
- (BOOL)transforBufferToBuffer:(DGCBEBuffer *)inputBuffer outputBuffer:(DGCBEBuffer *)outputBuffer;

//   {zh} / @brief 旋转 DGCBEBuffer     {en} @Briefing DGCBEBuffer 
//   {zh} / @param inputBuffer 输入 DGCBEBuffer     {en} /@param inputBuffer Enter DGCBEBuffer 
//   {zh} / @param outputBuffer 输出 DGCBEBuffer     {en} /@param outputBuffer output DGCBEBuffer 
//   {zh} / @param rotation 旋转角度，90/180/270     {en} /@param rotation angle, 90/180/270 
- (BOOL)rotateBufferToBuffer:(DGCBEBuffer *)inputBuffer outputBuffer:(DGCBEBuffer *)outputBuffer rotation:(int)rotation;

//   {zh} / @brief DGCBEBuffer 转 纹理     {en} /@Brief DGCBEBuffer, texture 
/// @param buffer DGCBEBuffer
- (id<BEGLTexture>)transforBufferToTexture:(DGCBEBuffer *)buffer;

//   {zh} / @brief DGCBEBuffer 转 UIImage     {en} @Briefing DGCBEBuffer to UIImage 
/// @param buffer DGCBEBuffer
- (UIImage *)transforBufferToUIImage:(DGCBEBuffer *)buffer;

#pragma mark - Texture to others

//   {zh} / @brief 纹理转 DGCBEBuffer     {en} /@brief texture to DGCBEBuffer 
//   {zh} / @param texture 纹理 ID     {en} /@param texture texture ID 
//   {zh} / @param widht 宽     {en} /@param widht 
//   {zh} / @param height 高     {en} /@param height 
//   {zh} / @param outputFormat 输出 DGCBEBuffer 格式，仅支持 RGBA/BGRA     {en} /@param outputFormat output DGCBEBuffer format, only supports RGBA/BGRA 
- (DGCBEBuffer *)transforTextureToBEBuffer:(GLuint)texture width:(int)widht height:(int)height outputFormat:(BEFormatType)outputFormat;

#pragma mark - UIImage to others

//   {zh} / @brief UIImage 转 DGCBEBuffer     {en} @Briefing UIImage to DGCBEBuffer 
/// @param image UIImage
- (DGCBEBuffer *)transforUIImageToBEBuffer:(UIImage *)image;

//   {zh} / @brief CIImage 转 CVPixelBufferRef     {en} @Briefing CIImage to CVPixelBufferRef
/// @param image CIImage
//   {zh} / @使用时注意返回值CVPixelBufferRef的释放    {en} @Pay attention to the release of the return value CVPixelBufferRef when using it
//    CVPixelBufferRef pixelBuffer = [self transforCIImageToCVPixelBuffer:image];
...
/// @param pixelBuffer CVPixelBuffer
- (DGCBEPixelBufferInfo *)getCVPixelBufferInfo:(CVPixelBufferRef)pixelBuffer;

//   {zh} / @brief 创建 DGCBEBuffer     {en} /@Briefing Create DGCBEBuffer 
...
/// @param format BEFormatType
- (DGCBEBuffer *)allocBufferWithWidth:(int)width height:(int)height bytesPerRow:(int)bytesPerRow format:(BEFormatType)format;
