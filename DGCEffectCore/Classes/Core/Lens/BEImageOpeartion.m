...
@interface DGCBEImageBufferOperation ()
@property (nonatomic, strong) NSMutableDictionary<NSString*, NSValue*>* pixelBufferPoolWithDifferentProperty;
@end

@implementation DGCBEImageBufferOperation

+ (instancetype) sharedInstance{
    static dispatch_once_t onceToken;
    static DGCBEImageBufferOperation *helper = nil;
    dispatch_once(&onceToken, ^{
        if (helper == nil){
            helper = [DGCBEImageBufferOperation new];
        }
    });
    return helper;
}
...
-(CVPixelBufferPoolRef)genPixelBufferPoolWith:(OSType)type heigth:(int)height width:(int)width{
    ...
    if (MTLCreateSystemDefaultDevice()) {
        [attributes setObject:[NSNumber numberWithBool:YES] forKey:(NSString*)kCVPixelBufferMetalCompatibilityKey];
    }
