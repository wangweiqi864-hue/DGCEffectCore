...
@interface DGCBEAlgorithmTask : NSObject
...
//   {zh} / 需要另外实现的，可以直接用工程中的 DGCBEAlgorithmResourceHelper 实现类，它可以拿到所有算法所需要的资源。     {en} If you need to implement it separately, you can directly use the BEAlgorithm ResourceHelper implementation class in the project, which can get all the resources needed by the algorithm. 
...
//   {zh} / 返回值也是每个算法不同的，返回结果定义在各算法的头文件中，如 DGCBEFaceAlgorithmResult。     {en} /The return value is also different for each algorithm, and the return result is defined in the header file of each algorithm, such as DGCBEFaceAlgorithmResult. 
/// @param buffer buffer
...
//   {zh} / 如 DGCBEFaceAlgorithmTask.FACE_280 。     {en} /As DGCBEFaceAlgorithmTask.FACE_280. 
...
// protected
- (BOOL)boolConfig:(DGCBEAlgorithmKey *)key orDefault:(BOOL)orDefault;
- (float)floatConfig:(DGCBEAlgorithmKey *)key orDefault:(float)orDefault;
